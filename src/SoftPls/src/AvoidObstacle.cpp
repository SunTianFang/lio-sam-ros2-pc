...
void CAvoidObstacle::UpdateGetSpeedType( void )
{
    ...
    int CenterCnt = 0;
    int WarningCnt = 0;
    float StopPointX  = 0.0;
    float StopPointY  = 0.0;
  ...
  //  printf("m_param size is %d,m_iAreaIndex %d,ip %s\n",m_param.size(),m_iAreaIndex,m_sLaserIP.c_str());
    ...
    #ifdef USE_BLACK_BOX
         FILE_BlackBox(LocBox, "SoftPls in stop area",", laser ip is: ",m_sLaserIP.c_str(),", AreaIndex is: ",m_iAreaIndex,", Stopnum: ",StopCnt,", one of points in stop area: x =",StopPointX,",  y=:",StopPointY);
    #endif
