...
{
    std::ifstream FileLocParm("/home/stf/lio-sam_all/ros2_pc/lio-sam-ros2-pc_add_communication_and_supported_livo_mid_360/src/LIO-SAM/src/RoboLocParm.json");
    Json::Reader Jreader;
...
bool CRoboLocClnt::CreateUdpCom()
{
    ...
    if (m_pLocUdpSocket == NULL)
    {
        ...
        // std::cout << "is m_pLocUdpSocket new: " << std::endl;
    }
    ...
    // std::cout << "bResult2: " << bResult << std::endl;
...
void CRoboLocClnt::ProcessReceiveData()
{
    ...
    // std::cout << "m_nRecvCount: " << m_nRecvCount << std::endl;
	while(RxReady())
    {
...
{
    // std::cout << " now is ReportRoboPose" << std::endl;
   // if(!(m_aCurWorkMode.load() == RLP_MODE_LOCALIZATION || m_aCurWorkMode.load() == RLP_MODE_MAPPING)){
    //    return false;
    //}
    ...
    // std::cout << "poseDataTx 1111_ nx: " << poseDataTx_.nX << " ny: " << poseDataTx_.nY << " ntheta: " << poseDataTx_.nTheta<<std::endl;
    ...
    int time_span = 0;
    // std::cout << "poseDataTx 2222 nx: " << poseDataTx_.nX << " ny: " << poseDataTx_.nY << " ntheta: " << poseDataTx_.nTheta<<std::endl;
    if(m_aCurWorkMode.load() == RLP_MODE_LOCALIZATION){
        ...
        if((cur_pose_time > m_LastPoseTime) || (time_span > 0 && time_span < ROBO_POSE_TOLERANCE_TIME)){
            ...
            // std::cout << "poseDataTx 3333 nx: " << poseDataTx_.nX << " ny: " << poseDataTx_.nY << " ntheta: " << poseDataTx_.nTheta<<std::endl;
...
//            poseDataTx_.nLegTheta = static_cast<int>(new_leg_pose.fThita * 1000.0);
    ...
    // std::cout << "poseDataTx last_ nx: " << poseDataTx_.nX << " ny: " << poseDataTx_.nY << " ntheta: " << poseDataTx_.nTheta<<std::endl;
    if(ClntReportPoseSe(this, poseDataTx_)){
        // std::cout << "ClntReportPoseSe is ok !!!sssssssssss" <<std::endl;
    }else{
        // std::cout << "ClntReportPoseSe is false !!!ffffffff"<< std::endl;
    }
                               ...
                               unsigned char chPosMode)
{
    {
        std::lock_guard<std::mutex> lock(data_mtx);
        std::cout << "poseDataTx aaaa_ nx: " << m_TxLocDataSet.m_PoseData.nX << " ny: " << m_TxLocDataSet.m_PoseData.nY << " ntheta: " << m_TxLocDataSet.m_PoseData.nTheta<<std::endl;
    ...
    std::cout << "SetRoboPose is ok" << std::endl;
    return true;
}
