...
bool CLaserAutoMapping::UpdateScan(sensor::CRawScan& rawScan)
{
    ...
    float acc_dis = odom_pst.DistanceTo(m_PreOriginalOdomPst);
...
// by lishen 开始建图
//
int CLaserAutoMapping::StartMapping()
{
...
bool CLaserAutoMapping::SaveMap()
{
    ...
    if ( nodeDatas.size() > 0 )
    {
        ...
        if(pRawMap->GetRawScans(pRawScans))
        {
           ...
           // std::cout << "SaveMap()   nodeDatas.size "<<nodeDatas.size() << std::endl;
           //  std::cout << "SaveMap()   transform_odom.size "<<transform_odom.size() << std::endl;

          //  std::cout << "SaveMap()   time_record.size "<<time_record.size() << std::endl;
                    ...
                    if((long int)nodestamp.tv_sec == sec && (long int)nodestamp.tv_usec == usec)
                    {
                       ...
                       // outfile1<<m_stepDatas[node.node_id].id<<": "<<pRawScans.at(i).point_cloud[0]->timestamp_raw<<endl;
                        ...
                        //   save opt dx               //?????????? if there are two lasers
                         ...
                         if(m_bCameraFlag && (i>=frozenNodeNum))
                         {
                             // dq VISION record Json
                             Json::Value root;
                             Json::Value laser;
                           ...
                           // laser["max_angle"] = Json::Value(180.0);
                            ...
                            img["idx"] = Json::Value(i-frozenNodeNum);

                            if((i-frozenNodeNum)<time_record.size())
                                img["time"] = Json::Value(time_record[i-frozenNodeNum]);
                            pose["y"] = Json::Value(pstRobot.y);
                            pose["x"] = Json::Value(pstRobot.x);
                            pose["yaw"] = Json::Value(pstRobot.fThita);
                            pose["time"] = Json::Value(time);
                             ...
                             if((i-frozenNodeNum)<time_record.size())
                             {
                                odom["y"] = Json::Value(transform_odom[i-frozenNodeNum].y);
                                odom["x"] = Json::Value(transform_odom[i-frozenNodeNum].x);
                                odom["yaw"] = Json::Value(transform_odom[i-frozenNodeNum].fThita);
                             }
                            odom["time"] = Json::Value(time);
                            root["laser"] = Json::Value(laser);
                            root["img"] = Json::Value(img);
   ...
   pRawScans.clear();
    ...
    if(m_bCameraFlag)
    {
       ...
       // std::cout<<"@@@@@@@  m_pCartoSlam->GetFrozenNodeNum() = "<<m_pCartoSlam->GetFrozenNodeNum()<<std::endl;
       // std::cout<<"@@@@@@@  img_record = "<<img_record.size()<<std::endl;
